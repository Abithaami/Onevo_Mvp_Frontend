...
/** Human-readable learning path for dashboard recap (from merged onboarding → brandData). */
function learningMethodSummary(brandData) {
  const url = brandData.websiteUrl?.trim();
  if (brandData.brandInputMode === 'website') {
    return url ? `From website · ${url}` : 'From website';
  }
...
export default function DashboardWorkspaceContent({ setup, onSetupChange, onOpenConnections, activeNav }) {
  ...
  const learningLine = learningMethodSummary(setup.brandData);
    ...
    return (
      <>
        ...
        <section className="dashboard-onboarding-recap" aria-labelledby="dashboard-onboarding-recap-title">
          ...
          <p className="dashboard-onboarding-recap__lede">Pulled from onboarding—edit details anytime under Brand DNA or Growth.</p>
